...
public class getGeneralSection extends AbstractBWTransactionalSection {
	...
	*/
    ...
    protected void initBindings() {
        getBindingManager().bind(url, getInput(), ElasticachePackage.Literals.GET__URL); 
        getBindingManager().bind(port, getInput(), ElasticachePackage.Literals.GET__PORT); 
        getBindingManager().bind(username, getInput(), ElasticachePackage.Literals.GET__USERNAME); 
        getBindingManager().bind(password, getInput(), ElasticachePackage.Literals.GET__PASSWORD); 
   	    // begin-custom-code
        // end-custom-code
    }
    ...
    protected Composite doCreateControl(final Composite root) {
        Composite parent = BWFieldFactory.getInstance().createComposite(root, 2);
   	    BWFieldFactory.getInstance().createLabel(parent, Messages.GET_URL, false);
   	    url = BWFieldFactory.getInstance().createTextBox(parent);

   	    BWFieldFactory.getInstance().createLabel(parent, Messages.GET_PORT, false);
   	    port = BWFieldFactory.getInstance().createSpinner(parent, 2, SWT.BORDER);

   	    BWFieldFactory.getInstance().createLabel(parent, Messages.GET_USERNAME, false);
   	    username = BWFieldFactory.getInstance().createTextBox(parent);

   	    BWFieldFactory.getInstance().createLabel(parent, Messages.GET_PASSWORD, false);
