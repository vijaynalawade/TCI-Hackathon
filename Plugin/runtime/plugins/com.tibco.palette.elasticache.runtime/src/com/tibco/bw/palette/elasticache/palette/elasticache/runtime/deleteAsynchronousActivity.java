...
import com.tibco.bw.palette.elasticache.palette.elasticache.runtime.pojo.delete.DeleteOutput;
...
public class deleteAsynchronousActivity<N> extends AsyncActivity<N> implements ElasticacheContants {
	...
	public void execute(N input, ProcessContext<N> processContext, AsyncActivityController asyncActivityController) throws ActivityFault {
        ...
        // add your own business code here
        // end-custom-code
		deleteActivityExecutor<N> runnable = new deleteActivityExecutor<N>(notifier, input, processContext);
	...
	class deleteActivityExecutor<A> implements Runnable {
		...
		public deleteActivityExecutor(AsyncActivityCompletionNotifier notifier, N input, ProcessContext<N> processContext) {
		...
		public void run() {
			...
			try {				
				MemcachedClientWrapper.deleteValue( getInputParameterStringValueByName(inputData,  processContext.getXMLProcessingContext(), "key")); 
	...
	protected <A> N evalOutput(N inputData, ProcessingContext<N> processingContext, Object data) throws Exception {
		...
		deleteOutput.setResponse("Deleted");
		N output = PaletteUtil.parseObjtoN(DeleteOutput.class, deleteOutput, processingContext, activityContext.getActivityOutputType().getTargetNamespace(), "deleteOutput");
		// begin-custom-code
        // add your own business code here
        // end-custom-code
	    return output;
	 ...
	 */		
	 protected N getOutputRootElement(ProcessingContext<N> processingContext) {
        ...
        try {
			builder.startElement(activityContext.getActivityOutputType().getTargetNamespace(), "deleteOutput", "ns0");
        try {
			} finally {
				builder.endElement();
			}
