...
public class getAsynchronousActivity<N> extends AsyncActivity<N> implements ElasticacheContants {
	...
	class getActivityExecutor<A> implements Runnable {
		...
		public void run() {
			...
			try {				
				...
				Object outputData =MemcachedClientWrapper.getValue( getInputParameterStringValueByName(inputData,  processContext.getXMLProcessingContext(), "key")); 
				N output = evalOutput(inputData, processContext.getXMLProcessingContext(), outputData);
	...
	protected <A> N evalOutput(N inputData, ProcessingContext<N> processingContext, Object data) throws Exception {
		...
		getOutput.setValue((String)data);
		N output = PaletteUtil.parseObjtoN(GetOutput.class, getOutput, processingContext, activityContext.getActivityOutputType().getTargetNamespace(), "getOutput");
		// begin-custom-code
        // add your own business code here
        // end-custom-code
	    return output;
