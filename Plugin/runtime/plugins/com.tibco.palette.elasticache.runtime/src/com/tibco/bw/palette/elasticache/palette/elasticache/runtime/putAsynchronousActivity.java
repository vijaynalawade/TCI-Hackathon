...
public class putAsynchronousActivity<N> extends AsyncActivity<N> implements ElasticacheContants {
	...
	class putActivityExecutor<A> implements Runnable {
		...
		public void run() {
			...
			try {				
				...
				MemcachedClientWrapper.setValue(getInputParameterStringValueByName(inputData,  processContext.getXMLProcessingContext(), "key"),0, getInputParameterStringValueByName(inputData,  processContext.getXMLProcessingContext(), "value")); 
	...
	protected <A> N evalOutput(N inputData, ProcessingContext<N> processingContext, Object data) throws Exception {
		...
		N output = PaletteUtil.parseObjtoN(PutOutput.class, putOutput, processingContext, activityContext.getActivityOutputType().getTargetNamespace(), "putOutput");
		// begin-custom-code
        // add your own business code here
        // end-custom-code
	    return output;
	 ...
	 */		
	 protected N getOutputRootElement(ProcessingContext<N> processingContext) {
        ...
        try {
			builder.startElement(activityContext.getActivityOutputType().getTargetNamespace(), "putOutput", "ns0");
        try {
			} finally {
				builder.endElement();
			}
