...
import com.tibco.bw.palette.elasticache.palette.elasticache.runtime.pojo.update.UpdateOutput;
...
public class updateAsynchronousActivity<N> extends AsyncActivity<N> implements ElasticacheContants {
	private ExecutorService threadPool = null;
	private final ConcurrentHashMap<String, Future<?>> executingTasks = new ConcurrentHashMap<String, Future<?>>();
	
	@Property
	public update activityConfig;
	...
	public void execute(N input, ProcessContext<N> processContext, AsyncActivityController asyncActivityController) throws ActivityFault {
        ...
        // add your own business code here
        // end-custom-code
		updateActivityExecutor<N> runnable = new updateActivityExecutor<N>(notifier, input, processContext);
	...
	class updateActivityExecutor<A> implements Runnable {
		...
		public updateActivityExecutor(AsyncActivityCompletionNotifier notifier, N input, ProcessContext<N> processContext) {
	...
	protected <A> N evalOutput(N inputData, ProcessingContext<N> processingContext, Object data) throws Exception {
		...
		updateOutput.setResponse("StringValue");
		N output = PaletteUtil.parseObjtoN(UpdateOutput.class, updateOutput, processingContext, activityContext.getActivityOutputType().getTargetNamespace(), "updateOutput");
		// begin-custom-code
        // add your own business code here
        // end-custom-code
	    return output;
	 ...
	 */		
	 protected N getOutputRootElement(ProcessingContext<N> processingContext) {
        ...
        try {
			builder.startElement(activityContext.getActivityOutputType().getTargetNamespace(), "updateOutput", "ns0");
        try {
			} finally {
				builder.endElement();
			}
